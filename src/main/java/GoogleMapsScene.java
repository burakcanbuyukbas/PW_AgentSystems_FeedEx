...
/*    public static GoogleMapsScene getInstance(){
    ...
    public void addMarker(double lat, double lng, String key, String label, int icon){
        Platform.runLater(()->{
            engine.executeScript("addMarker("+lat+","+lng+",'"+key+"',"+undefined(label)+","+icon+");");
        });
    }
    ...
    public void calcRoute(double start, double end, String mode){       //not sure if works
        Platform.runLater(()->{
            engine.executeScript("calcRoute("+start+","+end+",'"+mode+");");
        });
    }
